...
def _skip_if_port_853_unreachable(transport, exc):
...
def test_resolve_a_record(transport):
    try:
...
@pytest.mark.parametrize("transport", TRANSPORTS)
async def test_aresolve_a_record(transport):
    try:
        response = await transport.aresolve("example.com", "A")
    except py_doh_core.DohError as exc:
        _skip_if_port_853_unreachable(transport, exc)
        raise
...
def test_resolve_nxdomain_is_not_an_error():
...
def test_resolve_servfail_raises_doh_error():
...
def test_invalid_server_url_raises_doh_error():
    with pytest.raises(py_doh_core.DohError):
        py_doh_core.PyDohTransport("not-a-valid-url")


def test_unknown_record_type_raises_doh_error():
    with pytest.raises(py_doh_core.DohError):
        DOH_GET.resolve("example.com", "NOT_A_RECORD_TYPE")


def test_answers_authorities_additionals_are_populated_separately():
